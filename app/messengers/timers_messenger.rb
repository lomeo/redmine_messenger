class IssuesMessenger < RedmineMessenger::Base
  ...
  def status_available(messenger, status)
    if messenger.resume_when_become_online? and messenger.timer_running? and messenger.timer_paused_because_of_status_change?
      ...
      l(:messenger_command_timers_resumed_because_of_status_change, :issue => messenger.issue.subject)
    end
  end
  ...
  def status_unavailable(messenger, status)
    if messenger.pause_when_become_offline_or_away? and messenger.timer_running? and not messenger.timer_paused?
      ...
      l(:messenger_command_timers_paused_because_of_status_change, :issue => messenger.issue.subject)
    end
  end
  ...
  def resume(messenger, params = {})
    if messenger.timer_running?
      if messenger.timer_paused?
        messenger.timer_resume(params[:note])
        l(:messenger_command_timers_resumed, :issue => messenger.issue.subject)
      else
        l(:messenger_command_timers_not_resumed, :issue => messenger.issue.subject)
      end
    else
  ...
  def pause(messenger, params = {})
    if messenger.timer_running?
      unless messenger.timer_paused?
        ...
        l(:messenger_command_timers_paused, :issue => messenger.issue.subject)
      else
        if messenger.timer_paused_because_of_status_change?
          messenger.timer_pause(params[:note])
          l(:messenger_command_timers_paused, :issue => messenger.issue.subject)
        else
          l(:messenger_command_timers_not_paused, :issue => messenger.issue.subject)
        end
      end
    else
      l(:messenger_command_timers_not_running)
  ...
  def cancel(messenger, params = {})
    if messenger.timer_running?
      ...
      l(:messenger_command_timers_cancelled, :issue => messenger.issue.subject)
    else
      l(:messenger_command_timers_not_running)
    end
  end
  ...
  def finish(messenger, params = {})
    if messenger.timer_running?
      ...
      if messenger.timer_finish(params[:done_ratio], params[:note])
        l(:messenger_command_timers_finished, :issue => issue.subject)
      else
        l(:messenger_command_timers_not_finished, :issue => issue.subject)
      end
    else
      l(:messenger_command_timers_not_running)
    end
  ...
  def note(messenger, params = {})
    if messenger.timer_running?
      ...
      l(:messenger_command_timers_noted, :issue => messenger.issue.subject)
    else
      l(:messenger_command_timers_not_running)
    end
  end
  ...
  def status(messenger, params = {})
    if params[:issue_id] and params[:issue_id] > 0 and messenger.issue_id != params[:issue_id]
      if issue = Issue.find_by_id(params[:issue_id])
        ...
        responce = l(:messenger_command_timers_not_running_for_that_issue, :issue => issue.subject) << "\n"
      ...
      if messenger.timer_running?
        ...
        if messenger.timer_paused?
          responce = l(:messenger_command_timers_paused_status, :issue => messenger.issue.subject) << "\n"
        else
          responce = l(:messenger_command_timers_running_status, { :issue => messenger.issue.subject, :minutes => messenger.timer_to_minutes }) << "\n"
        end
        responce << status_for_issue(messenger, stats)
        responce
      else
  ...
  def status_for_issue(messenger, stats)
    ...
    responce = l(:messenger_command_timers_running_stats_time, { :hours_logged_by_you_today => logged_by_you_today, :hours_logged_by_you => logged_by_you, :hours_logged_by_all => logged_by_all }) << "\n"
    if estimated_time > 0
      responce << l(:messenger_command_timers_running_stats_done_with_estimation, { :done => done_ratio, :remaining_hours => rest_time, :estimated_hours => estimated_time })
    else
      responce << l(:messenger_command_timers_running_stats_done, :done => done_ratio)
    end
    responce
  end
